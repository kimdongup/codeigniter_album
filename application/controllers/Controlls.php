...
class Controlls extends CI_Controller {
	...
	function upload_photo()
	{
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						redirect('/sns/index.php/controlls/upload_photo');
					}
		...
		else
		{
			...
			document.location = '/sns/index.php/auth/login/';
			</script>";
			exit;
		}
	}
	...
	function facebook_upload()
	{
		...
		redirect('/sns/index.php/cotrolls/lists');
	}
	...
	function delete()
 	{
		...
		if( @$this->session->userdata('logged_in') == TRUE )
		{
			...
			{
				alert('본인이 작성한 글이 아닙니다.', '/sns/index.php/controlls/view/'.$this->uri->segment(3).'/page/'.$this->uri->segment(5));
				exit;
			}
			...
			{
				//삭제가 성공한 경우
				alert('삭제되었습니다.', '/sns/index.php/controlls/lists/page/'.$this->uri->segment(5));
				exit;
			}
			else
			{
				//삭제가 실패한 경우
				alert('삭제 실패하였습니다.', '/sns/index.php/controlls/view/'.$this->uri->segment(3).'/page/'.$this->uri->segment(5));
				exit;
			}
		}
		else
		{
			alert('로그인후 삭제하세요', '/sns/index.php/auth/login/');
			exit;
		}
 	}
	...
	function modify_photo()
	{
		...
		if( @$this->session->userdata('logged_in') == TRUE )
		{
			...
			if( $writer_id->user_id != $this->session->userdata('username') )
			{
				alert('본인이 작성한 글이 아닙니다.', '/sns/index.php/controlls/view/'.$id.'/page/'.$pages);
				exit;
			}
			...
			if ( $this->form_validation->run() == TRUE )
			{
				if ( !$this->input->post('subject', TRUE) AND !$this->input->post('contents', TRUE) )
				{
					...
					alert('비정상적인 접근입니다.', '/sns/index.php/controlls/lists/page/'.$pages);
					exit;
				}
				...
				redirect('/sns/index.php/controlls/lists'); exit;
				...
				{
					//글 작성 성공시 게시판 목록으로
					alert('수정되었습니다.', '/sns/index.php/controlls/lists/page/'.$pages);
					exit;
				}
				else
				{
					//글 수정 실패시 글 내용으로
					alert('다시 수정해 주세요.', '/sns/index.php/controlls/view/'.$id.'/page/'.$this->uri->segment(5));
					exit;
				}
		...
		{
			alert('로그인후 수정하세요', '/sns/index.php/auth/login/');
			exit;
		}
	}
